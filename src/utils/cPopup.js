    ...
    if (this.html) {
      ...
      var html = `
            ${
              isclose === false
                ? ""
                : '<div class="bx-popup-close"><span class="iconfont_DEU icon-guanbi">×</span></div>'
            }
...
class Tooltip {
  ...
  add(option) {
    ...
    var data = `data:image/svg+xml,
                <svg width="${width + 10}" height="${
      height + 30
    }" version="1.1" xmlns="http://www.w3.org/2000/svg">
                    
    <path d=" M${width + 5} 15 Q${width + 5} 5 ${
      width + 5 - 10
    } 5 L15 5 Q5 5 5 15 L5 ${height - 10 + 5} Q5 ${height + 5} 15 ${
      height + 5
    } L${width / 2 - 10 + 5} ${height + 5} ${width / 2 + 5} ${
      height + 20 + 5
    } ${width / 2 + 10 + 5} ${height + 5} ${width - 10 + 5} ${height + 5} Q${
      width + 5
    } ${height + 5} ${width + 5} ${height - 10 + 5} Z"
     fill="${this_.options.color}" stroke="${this_.options.stroke}" style="${
      this_.options.strokewidth
    }:3;opacity:${this_.options.opacity};stroke-opacity:0.8"></path>
    <path d=" M${width + 5} 15 Q${width + 5} 5 ${
      width + 5 - 10
    } 5 L15 5 Q5 5 5 15 L5 30 ${width + 5} 30 Z" 
    fill="${this_.options.color}" style="opacity:0.5;"></path>
<text x="15" y="22" fill="${this_.options.textcolor}" style="font-size:${
      this_.options.fontSize
    }; font-weight: 600;">${header}</text>
         <text fill="${this_.options.textcolor}" style="font-size: ${
      this_.options.fontSize
    };">
            ${cen}
      </text>
