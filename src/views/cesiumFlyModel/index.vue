<template>
  ...
  <div class="flex-box" style="top: 9%">加载单个飞行</div>
  ...
  handle.setInputAction((clickEvent: any) => {
    ...
    if (Cesium.defined(pickData)) {
      ...
      cesiumV.scene.preRender.addEventListener(() => {
        ...
        let canvasPosition = cesiumV.scene.cartesianToCanvasCoordinates(
          position,
          scratch,
        );
...
const addCzml = async (cesiumV: Cesium.Viewer) => {
  const dataSource = await cesiumV.dataSources.add(
...
const czml = [
  ...
  },
  {
    ...
    position: {
      ...
      cartographicDegrees: [
        0, 113.28328631492639, 23.111433215435753, 1500, 5, 113.50714976568543,
        23.694634169248147, 2000, 10, 113.6299135935181, 24.169871656793887,
        2500, 15, 113.70934901152907, 24.85320754694945, 3000, 20,
        113.65157704589103, 25.61746954483914, 3500, 25, 113.49150502960089,
        26.297929565617395, 4000, 30, 113.32225282991868, 26.919159149253858,
        4500, 35, 112.96275429879444, 27.54911441094461, 5000, 40,
        112.9449382777749, 28.234076869525722, 5500, 45, 112.8784720236336,
        28.543897602427435, 6000, 50, 112.92669944031508, 29.130070639697294,
        6500, 55, 113.14372281537442, 29.697212026021546, 7000, 60,
        113.43308669785722, 30.07354009292432, 7500, 65, 113.74053647919294,
        30.38085734849267, 8000, 70, 114.17458307056592, 30.583470562015506,
        8000, 75, 114.86182323969172, 30.899530905574252, 8000, 80,
        115.597291344067, 31.30215750288525, 8000, 85, 116.22424543023169,
        31.641504716189345, 7000, 90, 117.20407629814952, 31.812939039523783,
        6000, 95, 118.31626581796667, 32.256622831560406, 5500, 100,
        118.54273109813403, 33.02364879361495, 5000,
      ],
    },
  },
];
...
const singModelFly = () => {
  const julianDT = new Cesium.JulianDate();
  const start = Cesium.JulianDate.addHours(
    Cesium.JulianDate.fromDate(new Date(2025, 5, 9, 8)),
    8,
    julianDT,
  ); // 调整为北京时间
  const duration = 4;
...
const multiModel = async () => {
  const mulCzml = [
    ...
    },
    {
      ...
      position: {
        epoch: "2025-04-28T10:37:10Z",
        cartographicDegrees: [
          0, 113.28328631492639, 23.111433215435753, 1500, 5,
          113.50714976568543, 23.694634169248147, 2000, 10, 113.6299135935181,
          24.169871656793887, 2500, 15, 113.70934901152907, 24.85320754694945,
          3000, 20, 113.65157704589103, 25.61746954483914, 3500, 25,
          113.49150502960089, 26.297929565617395, 4000, 30, 113.32225282991868,
          26.919159149253858, 4500, 35, 112.96275429879444, 27.54911441094461,
          5000, 40, 112.9449382777749, 28.234076869525722, 5500, 45,
          112.8784720236336, 28.543897602427435, 6000, 50, 112.92669944031508,
          29.130070639697294, 6500, 55, 113.14372281537442, 29.697212026021546,
          7000, 60, 113.43308669785722, 30.07354009292432, 7500, 65,
          113.74053647919294, 30.38085734849267, 8000, 70, 114.17458307056592,
          30.583470562015506, 8000, 75, 114.86182323969172, 30.899530905574252,
          8000, 80, 115.597291344067, 31.30215750288525, 8000, 85,
          116.22424543023169, 31.641504716189345, 7000, 90, 117.20407629814952,
          31.812939039523783, 6000, 95, 118.31626581796667, 32.256622831560406,
          5500, 100, 118.54273109813403, 33.02364879361495, 5000,
        ],
      },
    },
    {
      ...
      position: {
        epoch: "2025-04-28T10:37:10Z",
        cartographicDegrees: [
          0, 113.7923396363662, 23.106990160005353, 1500, 5, 114.28048350624312,
          23.739311443864054, 2000, 10, 114.82468623840595, 24.484386889937966,
          2500, 15, 115.31806092785678, 25.663489133757224, 3000, 20,
          116.04843929319111, 26.710839538617932, 3500, 25, 116.069918991115,
          27.761318438407528, 4000, 30, 116.93720744983318, 28.8409109572179,
          4500, 35, 118.24769147243478, 29.715664149451882, 5000, 40,
          119.13560153143925, 30.787055778457216, 5500,
  ...
  const dataSource = await cesiumV.dataSources.add(
    Cesium.CzmlDataSource.load(mulCzml),
  );
...
const initModelMove = () => {
  ...
  setInterval(() => {
    ...
    const now = Cesium.JulianDate.addSeconds(
      start,
      Cesium.JulianDate.secondsDifference(Cesium.JulianDate.now(), start),
      new Cesium.JulianDate(),
    );
    dataModel1.timestamp = now;
    dataModel1.id = "123";
    const newDate = [dataModel1];
    updateData(newDate);
  }, 1000);
};
...
function updateData(list: any[]) {
  for (const data of list) {
    const { id, longitude, latitude, timestamp } = data;
    ...
    if (entityMap.has(id)) {
      ...
      const position = ship.position as Cesium.SampledPositionProperty;
      position.addSample(timestamp, currentPosition);
    } else {
      // 首次创建
      const position = new Cesium.SampledPositionProperty();
      position.setInterpolationOptions({
...
function calcHeading(p1: Cesium.Cartesian3, p2: Cesium.Cartesian3) {
